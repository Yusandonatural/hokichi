...
## 筆ポリゴンと農地ナビのピンは「ずれる」（実データでの知見）

奈良市東部の実データで照合した結果:

- 農地ナビのピンは **登記簿上の所在から置いた概ねの位置**で、筆ポリゴン（衛星画像の判読で作った区画）とは
  **中央値で 35m、遊休農地では 90m 前後ずれる**。耕作中の筆でもピンが区画の中に落ちるのは 3 割弱
- **荒廃した筆は筆ポリゴンに存在しないことが多い**（衛星画像で農地に見えないため判読対象から外れる）。
  遊休農地 43 筆のうち、区画の中にピンがあるものは 0、20m 以内に区画があるものは 5

したがって、筆ポリゴンは「この遊休農地の正確な形」ではなく **周辺の区画の見当をつける参考レイヤ**として扱う。
地図では細線で周辺の区画を描き、遊休農地そのものはピン位置の周りに面積相当の正方形（破線）で示す。
正確な形状と隣接関係は公図で確認する（`docs/02-地番を特定する.md`）。
